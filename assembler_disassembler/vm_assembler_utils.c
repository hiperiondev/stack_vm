...
bool to_u8(char *val, data_t *res) {
    ...
    const uint32_t sl = strtoul(val, &end, 10);
...
bool to_i8(char *val, data_t *res) {
    ...
    const int32_t sl = strtol(val, &end, 10);
...
int32_t strpos(char *str, char *substr, int offset) {
    ...
    if ((found = strstr(str, substr)) == NULL)
        return -1;
...
bool is_valid_name(char *str) {
    ...
    for (uint32_t n = 1; n < strlen(str); n++) {
        if (!(str[n] == '_' || (str[n] > 96 && str[n] < 123) || (str[n] > 64 && str[n] < 91) || (str[n] > 47 && str[n] < 58)))
            return false;
    }
...
void print_hex(uint8_t *hex, uint32_t qty, uint8_t spaces) {
    ...
    printf("%*s   PC  00 01 02 03 04 05 06 07 08 09  ------- ASCII ------- \n", spaces, " ");

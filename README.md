...
|****PUSH_0****            | 0x06 | none  | none   | push integer 0
|****PUSH_1****            | 0x07 | none  | none   | push integer 1
|****PUSH_CHAR****         | 0x08 |   u8  | none   | push integer value of char
...
|****PUSH_HEAP_OBJECT****  | 0x12 |  u32  | none   | push a heap object
|****NEW_ARRAY****         | 0x13 |  u16  | none   | create new array in heap and fill with n elements from stack. Push heap index
|****PUSH_ARRAY****        | 0x14 | none  | none   | push array to stack
|****GET_ARRAY_VALUE****   | 0x15 | @u16  | none   | get value from array
|****SET_ARRAY_VALUE****   | 0x16 | @u16  | none   | set value on array
|****ADD****               | 0x17 | none  | none   | add top and second element from stack
|****SUB****               | 0x18 | none  | none   | subtract top and second element from stack
|****MUL****               | 0x19 | none  | none   | multiply top and second element from stack
|****DIV****               | 0x1a | none  | none   | divide top and second element from stack
|****MOD****               | 0x1b | none  | none   | module from top and second element of stack
|****OR****                | 0x1c | none  | none   | logical or from top and second element of stack
|****AND****               | 0x1d | none  | none   | logical and from top and second element of stack
|****LT****                | 0x1e | none  | none   | lesser
|****LTE****               | 0x1f | none  | none   | lesser or equal
|****GT****                | 0x20 | none  | none   | greater
|****GTE****               | 0x21 | none  | none   | greater or equal
|****INC****               | 0x22 | none  | none   | increment number (if not number do nothing)
|****DEC****               | 0x23 | none  | none   | decrement number (if not number do nothing)
|****EQU****               | 0x24 | none  | none   | equality
|****NOT****               | 0x25 | none  | none   | binary not
|****SET_GLOBAL****        | 0x26 |  u32  | none   | set global variable (global 0xffffffff is indirect register)
|****GET_GLOBAL****        | 0x27 |  u32  | none   | get global variable (global 0xffffffff is indirect register)
|****GOTO****              | 0x28 | @u32  | none   | jump to pc
|****GOTOZ****             | 0x29 | @u32  | none   | jump to pc if top stack is zero
|****CALL****              | 0x2a |   u8  |  @u32  | call function in pc with u8 stack reserved for local variables
|****RETURN****            | 0x2b | none  | none   | return from function without values
|****RETURN_VALUE****      | 0x2c | none  | none   | return from function with value
|****CALL_FOREIGN****      | 0x2d |   u8  |  @u32  | call foreign function
|****GET_LOCAL****         | 0x2e |  u32  | none   | get local variable
|****GET_LOCAL_FF****      | 0x2f |   u8  | none   | get local variable (first 0xff)
|****SET_LOCAL****         | 0x30 |  u32  | none   | set local variable
|****SET_LOCAL_FF****      | 0x31 |   u8  | none   | set local variable (first 0xff)
|****GET_RETVAL****        | 0x32 | none  | none   | push in stack value from function return
|****TO_TYPE****           | 0x33 |   u8  | none   | convert value to new type
|****DROP****              | 0x34 | none  | none   | drop top of stack
|****HALT****              | 0x35 |   u8  | none   | stop vm
